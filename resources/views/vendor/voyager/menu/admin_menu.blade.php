...
@foreach ($items as $item)
    @php
        ...
        if(!$item->children->isEmpty())
        {
            ...
            $linkAttributes = 'href="#' . str_slug($transItem->id, '-') .'-dropdown-element" data-toggle="collapse" aria-expanded="'. (in_array('active', $listItemClass) ? 'true' : 'false').'"';
    ...
    <li class="{{ implode(" ", $listItemClass) }}">
        ...
        @if($hasChildren)
            <div id="{{ str_slug($transItem->id, '-') }}-dropdown-element" class="panel-collapse collapse {{ (in_array('active', $listItemClass) ? 'in' : '') }}">
